...
namespace Object {
	...
	void initPatches(float pathTime)
	{
		...
		*/
		...
		// ============================================================
// ТРАНЗИТ — РАЗДВОЕНИЕ И СХОЖДЕНИЕ
// ============================================================


// ----------------------------
// ОСНОВНАЯ ВХОДНАЯ ЧАСТЬ
// ----------------------------

		NewLine();

		AddPointToLine({ 92,  0,   0 });
		AddPointToLine({ 180,  0,  -25 });
		AddPointToLine({ 184,  0,  23 });
		AddPointToLine({ 147,  0,  30 });
		...
		AddPointToLine({ 150,   0,  25 });
		AddPointToLine({ 150,   2,  30 });
		AddPointToLine({ 151,   6,  35 });
		AddPointToLine({ 153,  12,  40 });
		AddPointToLine({ 156,  18,  45 });

		AddPointToLine({ 160,  24,  50 });
		AddPointToLine({ 165,  27,  55 });
		AddPointToLine({ 170,  28,  60 });

		AddPointToLine({ 174,  26,  65 });
		AddPointToLine({ 177,  20,  70 });
		AddPointToLine({ 178,  14,  75 });

		AddPointToLine({ 176,   8,  80 });
		AddPointToLine({ 171,   3,  85 });
		AddPointToLine({ 160,   1,  90 });

		AddPointToLine({ 140,   0,  95 });
		AddPointToLine({ 100,   0, 100 });
		...
		AddPointToLine({ 150,   0,  25 });
		AddPointToLine({ 150,  -2,  30 });
		AddPointToLine({ 149,  -6,  35 });
		AddPointToLine({ 147, -12,  40 });
		AddPointToLine({ 144, -18,  45 });

		AddPointToLine({ 140, -24,  50 });
		AddPointToLine({ 135, -27,  55 });
		AddPointToLine({ 130, -28,  60 });

		AddPointToLine({ 126, -26,  65 });
		AddPointToLine({ 123, -20,  70 });
		AddPointToLine({ 122, -14,  75 });

		AddPointToLine({ 124,  -8,  80 });
		AddPointToLine({ 129,  -3,  85 });
		AddPointToLine({ 135,  -1,  90 });

		AddPointToLine({ 147,   0,  95 });
		AddPointToLine({ 163,   0, 100 });


		// ============================================================
		// ОСНОВНАЯ ВЫХОДНАЯ ЧАСТЬ
		// ============================================================

		NewLine();

		AddPointToLine({ 152,  0,  95 });
		AddPointToLine({ 139,  0, 108 });
		AddPointToLine({ 147,  0, 127 });
		AddPointToLine({ 200,  0, 125 });
